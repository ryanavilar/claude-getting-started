...
Installs and configures **Claude Code CLI**, **Bun**, **RTK**, **tmux**, and an optional **Telegram plugin** — then auto-starts Claude inside a persistent tmux session on every login.
...
| 5 | **tmux** | Installs via Homebrew (macOS) or apt/dnf/pacman (Linux) |
| 6 | **Telegram** | *(optional)* Registers a Telegram bot as a Claude MCP server |
| 7 | **Auto-start** | Creates a launchd agent (macOS) or systemd service (Linux) that runs `tmux new -s claude` with Claude on login |
